...
class opExecutionFilter extends sfExecutionFilter
{
  ...
  protected function handleSsl($actionInstance)
  {
    ...
    if (sfConfig::get('op_use_ssl', false) && $this->isFirstCall())
    {
      ...
      elseif (opSslUtil::isRequiredAction(sfConfig::get('sf_app'), $moduleName, $actionName))
      {
        if (!$request->isSecure())
        {
          $baseUrl = sfConfig::get('op_ssl_base_url');
      ...
      elseif (!opSslUtil::isSelectableAction(sfConfig::get('sf_app'), $moduleName, $actionName) && $request->isSecure())
      {
